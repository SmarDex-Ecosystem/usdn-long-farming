...
interface IUsdnLongFarming is IUsdnLongFarmingTypes, IUsdnLongFarmingErrors, IUsdnLongFarmingEvents {
     ...
     * @dev If the position is active (not liquidated), the rewards are sent to the position's owner and the position is
     * withdrawn. If the position has been liquidated on the USDN protocol, the rewards are distributed to `msg.sender`
     * and `DEAD_ADDRESS`, and the position is deleted.
     * @param tick The tick of the position.
     * @param tickVersion The version of the tick.
     * @param index The index of the position inside the tick.
